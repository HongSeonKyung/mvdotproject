 ...
 <%
  if (request.getMethod() == "POST") {
	  ...
	  if (id == null || pwd == null || id.length() == 0 || pwd.length() == 0) {
		  %>
		   <div class="error"><span style= "color:red";>아이디와 비밀번호를 입력하세요.</span></div>
		  <%
	  } else if (id.equals("nanhee") && pwd.equals("12345")) {
		  // 로그인 성공
	      session.setAttribute("userId", "nanhee");
	      session.setAttribute("userName", "조난희");	
	      response.sendRedirect("ex2.jsp");	      
		 } else {
		  %>
		   <div class="error"><span style= "color:red";> 로그인할 수 없습니다. 입력내용을 확인 후 다시 시도해 주십시오. </span></div>
		  <%
	  }
